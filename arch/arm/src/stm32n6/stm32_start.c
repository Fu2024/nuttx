...
void __start_c(void)
{
  ...
  /* Match CubeN6 RIF attributes for the board UART pads before pin muxing. */

#ifdef CONFIG_STM32_USART1
  modifyreg32(STM32_GPIOE_SECCFGR, 0, GPIO_PIN5 | GPIO_PIN6);
  putreg32(0xfb9f, STM32_GPIOE_PRIVCFGR);
#endif
...
#ifdef CONFIG_STM32_USART1
  /* Match the STM32CubeN6 DNN647 UART setup: USART1 kernel clock is CLKP
   * (64 MHz on this board). */

  putreg32(RCC_CCIPR13_USART1SEL_CLKP, STM32_RCC_CCIPR13);
#endif

  stm32_lowsetup();

#ifdef CONFIG_DNN647_BOOT_TRACE
  arm_lowputs("\r\nDNN647 B1 lowsetup\r\n");
#endif

#ifdef CONFIG_STM32_USART1
  /* stm32_configgpio() applies the pin's default privilege from the
   * GPIO encoding.  Re-apply the CubeN6 RIF attributes afterwards so
   * the non-secure USART driver can access PE5/PE6. */
  modifyreg32(STM32_GPIOE_SECCFGR, 0, GPIO_PIN5 | GPIO_PIN6);
  putreg32(0xfb9f, STM32_GPIOE_PRIVCFGR);
#endif
...
#ifdef CONFIG_DNN647_BOOT_TRACE
  arm_lowputs("DNN647 B2 earlyserial\r\n");
#endif

  stm32_board_initialize();

#ifdef CONFIG_DNN647_BOOT_TRACE
  arm_lowputs("DNN647 B3 boardinit\r\n");
#endif

#ifdef CONFIG_STM32_USART1
  /* Early serial init may re-apply the GPIO defaults; ensure the final
   * attributes match CubeN6 (secure, non-privileged) before nx_start(). */
  modifyreg32(STM32_GPIOE_SECCFGR, 0, GPIO_PIN5 | GPIO_PIN6);
  putreg32(0xfb9f, STM32_GPIOE_PRIVCFGR);
#endif
