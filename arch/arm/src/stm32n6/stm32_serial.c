...
#  endif
  ...
  .apbclock      = 64000000,             /* USART1SEL=CLKP on DNN647 */
...
static void stm32serial_setformat(struct uart_dev_s *dev)
{
  ...
  usartdiv16 = (priv->apbclock + (priv->baud >> 1)) / priv->baud;
...
static int stm32serial_setup(struct uart_dev_s *dev)
{
  ...
  /* Clear status left by the boot ROM/FSBL before enabling NuttX
   * interrupts.  On STM32N6 an uncleared IDLE flag can keep the USART IRQ
   * asserted even when RXNE is not set, which causes an interrupt storm
   * before the first real character is received. */
  ...
  stm32serial_putreg(priv, STM32_USART_RQR_OFFSET, USART_RQR_RXFRQ);
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      /* IDLE is not used by the NuttX serial upper-half, but it may be left
       * asserted by the boot chain.  Clear it explicitly so it cannot keep
       * the shared USART IRQ asserted without RXNE data. */
...
static void stm32serial_rxint(struct uart_dev_s *dev, bool enable)
{
  ...
  /* The line is normally idle while the console is being opened.  On
   * STM32N6 that can re-assert IDLE between setup() and this point.  Clear it
   * while interrupts are still masked, immediately after RXNEIE is written,
   * so enabling the RX interrupt cannot create an empty IRQ storm. */
...
static void stm32serial_send(struct uart_dev_s *dev, int ch)
{
  ...
  /* STM32N6 maps TXE/TXFNF to a status bit that can remain asserted while
   * the just-written byte is still being transferred.  Clear the stale TC
   * state and wait for this byte to complete before the upper-half sends the
   * next byte.  This prevents consecutive TDR writes from overwriting data. */
  stm32serial_putreg(priv, STM32_USART_ICR_OFFSET, USART_ICR_TCCF);
  stm32serial_putreg(priv, STM32_USART_TDR_OFFSET, (uint32_t)ch);
  while ((stm32serial_getreg(priv, STM32_USART_ISR_OFFSET) & USART_ISR_TC) == 0)
    {
    }
}
